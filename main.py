        ...
        for j in range(1, len(cards)):
            ...
            try:
                with open(jsonPath, 'r', encoding='utf-8') as file:
                    data = json.load(file)
                    if helper_functions.checkIfExists(data, parsed_card_serial) is False:
                        data["cardData"].append(parsed_card.asDict())
                    else:
                        print('Card with serial ' + parsed_card.serial.serialNumber + ' exists.')
            ...
            print(current_card_output.substitute(current_card_count=current_card_count, current_card_serial=parsed_card_serial))
